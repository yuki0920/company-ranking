            ...
            return (
              <li key={index} className='border-b p-2'>
                <div className='flex w-full justify-between mb-1'>
                  <Link
                    href={{
                      pathname: `/${lang}/companies/${company.securityCode}`,
                    }}
                  >
                    <span className='link-text'>
                      {limitNameLength(company.securityName, company.securityNameEn, lang, 25)}
                    </span>
                  </Link>
                  <small className=''>
                    {markets[company.marketId as keyof typeof markets]}・
                    {industries[company.industryCode as keyof typeof industries]}
                  </small>
                </div>
                <dl className='grid grid-cols-2 gap-x-4 mb-0 text-sm'>
                  <dt className='col-span-1'>{dict.rank}</dt>
                  <dd className='col-span-1 text-right'>
                    {from + index}
                    {dict.rankUnit}
                  </dd>
                  <dt className='col-span-1'>{dict.sales}</dt>
                  <dd className='col-span-1 text-right'>
          ...
          <thead>
            <tr className='text-left'>
              ...
              <th className='text-right'>
                {dict.sales}({dict.salesUnit})
              </th>
              <th className='text-right'>
                {dict.salary}({dict.salaryUnit})
              </th>
            </tr>
          </thead>
          <tbody>
                ...
                return (
                  <tr key={index} className='hover'>
                    ...
                    <td className='link-text'>
                      ...
                      >
                        {lang == "en" ? company.securityNameEn : company.securityName}
                      </Link>
                    </td>
                    ...
                    <td className='text-right'>
                      {numberWithDelimiter(divide_1_000_000(company.netSales))}
                    </td>
                    <td className='text-right'>
                      {numberWithDelimiter(divide_1_000(company.averageAnnualSalary))}
                    </td>
